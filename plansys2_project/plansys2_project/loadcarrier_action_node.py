...
class LoadCarrierAction(ActionExecutorClient):

    def __init__(self, robot_name):
        super().__init__(f'{robot_name}_loadcarrier_action_node', 0.3)
        self.progress_ = 0.0
    ...
    def do_work(self):
        print(f'Running LoadcarrierAction for robot: {self.robot_name}')
        #self.send_feedback(1.0, 'Loading box completed')  # Send final feedback since there's only one update
        self.finish(True, 1.0, 'LoadcarrierAction completed')  # Finish the action
        self.progress_ = 0.0  # Reset progress for future actions
        print('running3')
        self.get_logger().info(f'LoadcarrierAction completed')
        print('running10')
