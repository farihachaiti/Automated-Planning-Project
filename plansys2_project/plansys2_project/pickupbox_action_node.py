...
class PickupboxAction(ActionExecutorClient):

    def __init__(self, robot_name):
        super().__init__(f'{robot_name}_pickupbox_action_node', 0.3)
        self.progress_ = 0.0
    ...
    def do_work(self):
        ...
        print(f'Running PickupboxAction for robot: {self.robot_name}')
        #self.send_feedback(1.0, 'Pickupbox completed')  # Send final feedback since there's only one update
        self.finish(True, 1.0, 'Picking up boxes completed')  # Finish the action
        self.progress_ = 0.0  # Reset progress for future actions
        print('running3')
        self.get_logger().info(f'Picking up boxes completed')
        print('running10')
